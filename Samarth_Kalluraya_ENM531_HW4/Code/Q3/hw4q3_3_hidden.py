...
def normalize(data):            #standardize the data
    man=data.mean(0)
    std=data.std(0)
    y=data-man
    z=y/std
...
def forward_pass(x,w1,w2,w3,w4):                        #function takes input data and weights and computes the forward pass
    n=x.shape[0]
    ones_nx1=np.ones([n,1])
    x=np.concatenate((x, ones_nx1), axis=1)    
    HL1=relu(np.matmul(x,w1))
    HL1=np.concatenate((HL1, ones_nx1), axis=1) 
    HL2=relu(np.matmul(HL1,w2))
    HL2=np.concatenate((HL2, ones_nx1), axis=1)
    HL3=relu(np.matmul(HL2,w3))
    HL3=np.concatenate((HL3, ones_nx1), axis=1)
    y=sigmoid(np.matmul(HL3,w4))
    return y

def loss_func(w1,w2,w3,w4,data):                        #function takes all data and weights and computes the loss
    x=data[:,:-1]
    y=data[:,-1]
    eps=10**(-8)
...
def initialize_weights(x_y,z,hl1_count,hl2_count,hl3_count):                         #Initializes the weights 
    
    w1=np.random.normal(0,(2/(x_y.shape[1]+hl1_count)),(x_y.shape[1]+1,hl1_count))
    w2=np.random.normal(0,(2/(hl1_count+hl2_count)),(hl1_count+1,hl2_count))
    w3=np.random.normal(0,(2/(hl2_count+hl3_count)),(hl2_count+1,hl3_count))
    w4=np.random.normal(0,(2/(hl3_count+z.shape[1])),(hl3_count+1,z.shape[1]))
    return w1,w2,w3,w4
...
class AdamOptimizer:                #implements neural network and adam optimizer
    ...
    def __init__(self, x, y, w1, w2, w3, w4, loss_func, tunable_params):                       #*****************
      ...
      self.grad_func_w2=grad(self.loss_func,argnum=1)
      self.grad_func_w3=grad(self.loss_func,argnum=2)
      self.grad_func_w4=grad(self.loss_func,argnum=3)
...
#                
    ...
    def update_weights(self,weight,mn,vn,gn,j):     #runs adam optimizer and updates weights, and returns mn and vn of respective weights
        ...
        weight=weight-(self.tp.l*mnhat)/(np.sqrt(vnhat)+0.0000001)
        return (weight,mn,vn)
#    
    ...
    def find_weights(self):                       #trains the model on the neural network
        ...
        while i<self.tp.iter+1:
            ...
            for batchdata in self.make_batch():
                ...
                self.w3=np.squeeze(self.w3)
                self.w4=np.squeeze(self.w4)
                
                gn1=self.grad_func_w1(self.w1,self.w2,self.w3,self.w4,batchdata)        #generate gradient
                gn2=self.grad_func_w2(self.w1,self.w2,self.w3,self.w4,batchdata)        #generate gradient
                gn3=self.grad_func_w3(self.w1,self.w2,self.w3,self.w4,batchdata)        #generate gradient
                gn4=self.grad_func_w4(self.w1,self.w2,self.w3,self.w4,batchdata)        #generate gradient
                
                (self.w1,self.tp.mn1,self.tp.vn1)=self.update_weights(self.w1,self.tp.mn1,self.tp.vn1,gn1,i)        #Update weight, mn, vn
                (self.w2,self.tp.mn2,self.tp.vn2)=self.update_weights(self.w2,self.tp.mn2,self.tp.vn2,gn2,i)        #Update weight, mn, vn
                (self.w3,self.tp.mn3,self.tp.vn3)=self.update_weights(self.w3,self.tp.mn3,self.tp.vn3,gn3,i)        #Update weight, mn, vn
                (self.w4,self.tp.mn4,self.tp.vn4)=self.update_weights(self.w4,self.tp.mn4,self.tp.vn4,gn4,i)        #Update weight, mn, vn
                
                self.loss_array=np.concatenate((self.loss_array,[self.loss_func(self.w1,self.w2,self.w3,self.w4,self.data)]),axis=0)    #store value of loss at iteration
...
if __name__ == "__main__":
    ...
    (w1,w2,w3,w4)=initialize_weights(train_x,train_y_exact,Layer1,Layer2,Layer3)                      #initialize weights
    
    
    #( data, loss_func,mn,vn,b1,b2,batch_size, learning_rate,iteration)
    adam=AdamOptimizer(train_x, train_y_exact, w1, w2, w3, w4, loss_func, tuning_params)              #create object
    adam.find_weights()                                                                               #train      
    (final_w1,final_w2,final_w3,final_w4)=adam.get_weights()                      #get trained weights
