...
def forward_pass(x,w1,w2):                                            #function takes input data and weights and computes the forward pass
    n=x.shape[0]
    ones_nx1=np.ones([n,1])
    x=np.concatenate((x, ones_nx1), axis=1)    
    HL1=relu(np.matmul(x,w1))
    HL1=np.concatenate((HL1, ones_nx1), axis=1) 
    y=sigmoid(np.matmul(HL1,w2))
    return y

def loss_func(w1,w2,data):                                 #function takes all data and weights and computes the loss
    x=data[:,:-1]
    y=data[:,-1]
    eps=10**(-8)
...
def initialize_weights(x_y,z,hl1_count):                             #Initializes the weights  

    w1=np.random.normal(0,(2/(x_y.shape[1]+hl1_count)),(x_y.shape[1]+1,hl1_count))
    w2=np.random.normal(0,(2/(hl1_count+z.shape[1])),(hl1_count+1,z.shape[1]))
    return w1,w2
...
class AdamOptimizer:              #implements neural network and adam optimizer
    ...
    def __init__(self, x, y, w1, w2, loss_func, tunable_params):                       #*****************
    ...
    def find_weights(self):                                              #trains the model on the neural network
        ...
        while i<self.tp.iter+1:
            ...
            for batchdata in self.make_batch():
                ...
                gn1=self.grad_func_w1(self.w1,self.w2,batchdata)        #generate gradient
                gn2=self.grad_func_w2(self.w1,self.w2,batchdata)        #generate gradient
                ...
                self.loss_array=np.concatenate((self.loss_array,[self.loss_func(self.w1,self.w2,self.data)]),axis=0)   #store value of loss at iteration
#                #print(self.loss_func(self.params,self.data))
                i=i+1
                if i%100==0:
                    print(i)
#                    print(self.loss_array[-1])
#            if self.loss_array[-1]<500:
#                self.tp.iter=i-1
#                break        #return(self.params) 
...
if __name__ == "__main__":
    ...
    (w1,w2)=initialize_weights(train_x,train_y_exact,Layer1)                      #initialize weights
    
    
    #( data, loss_func,mn,vn,b1,b2,batch_size, learning_rate,iteration)
    adam=AdamOptimizer(train_x, train_y_exact, w1, w2, loss_func, tuning_params)                   #create object
