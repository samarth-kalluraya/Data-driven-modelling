...
def forward_pass(x,w1,w2,w3):                                      #function takes input data and weights and computes the forward pass
    n=x.shape[0]
    ...
    y=sigmoid(np.matmul(HL2,w3))
    return y

def loss_func(w1,w2,w3,data):                            #function takes all data and weights and computes the loss
    x=data[:,:-1]
    y=data[:,-1]
    eps=10**(-8)
    y_pred=forward_pass(x,w1,w2,w3)
    loss=-(np.sum(y*np.log(y_pred+eps)+(1-y)*np.log(1-y_pred+eps)))
    return loss

def initialize_weights(x_y,z,hl1_count,hl2_count):                     #Initializes the weights  
    ...
    w3=np.random.normal(0,(2/(hl2_count+z.shape[1])),(hl2_count+1,z.shape[1]))
    return w1,w2,w3
...
class AdamOptimizer:              #implements neural network and adam optimizer
    ...
    def __init__(self, x, y, w1, w2, w3, loss_func, tunable_params):                       #*****************
    ...
    def find_weights(self):                                              #trains the model on the neural network
        ...
        while i<self.tp.iter+1:
            ...
            for batchdata in self.make_batch():
                ...
                self.loss_array=np.concatenate((self.loss_array,[self.loss_func(self.w1,self.w2,self.w3,self.data)]),axis=0)   #store value of loss at iteration
...
if __name__ == "__main__":
    ...
    (w1,w2,w3)=initialize_weights(train_x,train_y_exact,Layer1,Layer2)                      #initialize weights
    
    
    #( data, loss_func,mn,vn,b1,b2,batch_size, learning_rate,iteration)
    adam=AdamOptimizer(train_x, train_y_exact, w1, w2, w3,loss_func, tuning_params)                   #create object
    adam.find_weights()                                                                               #train
    (final_w1,final_w2,final_w3)=adam.get_weights()                      #get trained weights
